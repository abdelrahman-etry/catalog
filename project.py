...
def deleteRestaurant(restaurant_id):
    ...
    if request.method == 'POST':
        ...
        session.commit()
        return redirect(
            url_for('showAllRestaurants', restaurant_id=restaurant_id))
    else:
        return render_template(
...
def catalogJSON():
    ...
    return jsonify(
                Restaurants=[c.serialize for c in restaurants],
                MenuItem=[i.serialize for i in items]
            )

# API endpoints for all items of a specific category.
@app.route('/<category_name>/items.json')
def itemsJSON(category_name):
    restaurants = session.query(Restaurant).filter_by(name=category_name).one()
    items = session.query(MenuItem).filter_by(restaurant=restaurants).all()
